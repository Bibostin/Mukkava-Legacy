...
while True:  #infinite loop
    message_length = symetric.decrypt(client_socket.recv(encryption_length_headersize))  #fetch a portion of data corosponding to the length of a encrypted headers length (in this example all headers are 44 bytes)
    if not len(message_length):  #If message length doesn't have a length, the server socket terminated softly and we should exit or handle for this.
        client_socket.close()
        break
    message_length = int(message_length.decode("utf-8").strip())  #The length of the rest of the audio data is the value of the previously fetched header, decoded from utf-8, strpped of whitespace and converted to an integer value
