...
def log_setup(file, screen, chat):
    ...
    if file:  # if file is true, we need to log debug messages to a file
        file_logger = logging.FileHandler(f'logs/sys-log/log-{datetime.datetime.now()}', mode='w', encoding='utf-8')  # Setup file logging to sys text files
    ...
    if chat:  # if chat is true, we need to log chat messages to a file
        chat_file_logger = logging.FileHandler(f'logs/chat-log/log-{datetime.datetime.now()}', mode='w', encoding='utf-8')  # Setup file logging to chat text files
