...
class TCPStack:  # IPv4 TCP Socket stack for receiving text and command packets
    def __init__(self, listen_port, connect_port, symetricphrase, username, initial_address=None):
        ...
        print(f"<:MAIN:TCP server started on {socket.gethostbyname(socket.gethostname())}:{self.listen_port} waiting for new connections")
        ...
        print(f"<:MAIN:Inbound Handler started")
        ...
        print(f"<:MAIN:Outbound Handler started")
        ...
        while True:
            self.inbound_socket_processor(input())
    ...
    def outbound_socket_handler(self, address, propagate_peers=False):  # A handler for generating OUTBOUND  (client -> server) socket data streams
        ...
        print(f"<:OUTh:Connected to remote server at {address}:{self.connect_port}")
